...
real network call (registered into the real registry AS "fantastic-jobs" for
the test's duration via monkeypatch.setitem, matching this repo's
provider-pluggable design) so no HTTP happens and the fetched `limit` is
directly observable.

v1.2: the fake is registered under the REAL "fantastic-jobs" id (not an
arbitrary "fake-fj") because `config.provider_config_block` now resolves a
provider's own config block by NAME for the two built-in providers
(fantastic-jobs -> cfg.api, fantastic-jobs-actor -> cfg.apify) and by its own
`providers:` entry for everything else — the weekly-quota-guard arithmetic
these tests assert on is specifically Fantastic Jobs' `api:` block, so the
fake must resolve the same way the real provider does."""
